...
public class SkeletalTrackingProvider : BackgroundDataProvider
{
    ...
    public SkeletalTrackingProvider(int id, Device device) : base(id)
    {
        kinect = device;
    ...
    protected override void RunBackgroundThreadAsync(int id, CancellationToken token)
    {
        try
        {
            ...
            UnityEngine.Debug.Log("Open K4A device successful. id " + id + "sn:" + kinect.SerialNum);

            var deviceCalibration = kinect.GetCalibration();
            ...
            using (Tracker tracker = Tracker.Create(deviceCalibration, new TrackerConfiguration() { ProcessingMode = TrackerProcessingMode.Gpu, SensorOrientation = SensorOrientation.Default }))
            {
                ...
                {
                    using (Capture sensorCapture = kinect.GetCapture())
                    {
